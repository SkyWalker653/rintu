...
function createNewRepository(){
	...
	RepositoryService.saveRepository(repository, $('#pmSAPCode').val() , $('#spocSAPCode').val() , $('#alternateApproverSapCode').val() , $('#serviceStartDate').val()   ,  $('#serviceEndDate').val()  , function(p){
		$('#switchDefination').tab('show');
	});
...
function addGroupToRepository(){
	...
	if($('#groupNameTemp').val() == null || $('#groupNameTemp').val() ==''){
		return;
	}
	...
	RepositoryService.addGroupToRepository( $('#id').val(), gName ,   function(p){
		...
		if(p != 0 || p != null){
			...
			$('#'+gName+'_action').html('<a href="#" style="color: red;" onclick="removeFromGroupList(\''+p+'\')" > <span title="Duplicate Group" class="glyphicon glyphicon-alert"></span> </a>');
		}
...
function bulkAddToUsers() {
	
	var bulkIds = $('#bulkIdCopy_field').val();
	var bulkIdArray = bulkIds.split("\n");
	
	dwr.engine.setAsync(false);
	
	if($("#bulkIdCopy_login_id").is(':checked')){
		
		for(var i = 0 ; i < bulkIdArray.length ; i++){
			fetchUserBySapCode( bulkIdArray[i] );
			addUserToRepository();
		}
		
		$('#bulkIdCopyModal').modal('hide');
		
	}else if($("#bulkIdCopy_login_id").is(':checked')){
...
function bulkAddToGroups(){
	...
	var bulkGroups = $('#bulkGroupCopy_field').val();
	var bulkGroupsArray = bulkGroups.split("\n");
	
	$('#bulkGroupCopyModal').modal('hide');
	
	for(var i = 0 ; i < bulkGroupsArray.length ; i++){
		
		if(bulkGroupsArray[i] == null || bulkGroupsArray[i] ==''){
			continue;
		}
		
		$('#groupNameTemp').val(bulkGroupsArray[i]);
		addGroupToRepository();
	}
	
	$('#groupNameTemp').val('');
	$('#bulkGroupCopy_field').val('');
	
	dwr.engine.setAsync(true);
}

function createDefaultGroups(){
	$('#bulkGroupCopy_field').val('PM\nCC\nQA\nDevelopers\nTesters');
}

</script>
<form action="<%= request.getContextPath() %>/svn/restoration/submitforapproval" method="post">
<div>
  ...
  <div class="tab-content">
	...
	<span style="margin-left: 10%; font-weight: bold;">Please provide the following project details to create new repository</span>
	<input type="hidden" id="id"  name="id" value=${repository.id} >
	<table border="1" style="width: 80%;">
	
		<tr class="active">
			<td colspan="2" style="height: 52px;" valign="middle" align="center">
				
				<span style="float: left; margin-left: 10px;"><b>Remedy ID : </b>${repository.incidentNumber}</span>
				
				<span style=""><b>Repository ID : </b>${repository.repoId}</span>
				
				<span style="float: right; margin-right: 10px;"><b>Created on : </b>${repository.repoCreatedOn.time}</span>
	...
	<tr>
		...
		<table border="0" style="width: 90%; height: 400px; margin-left: 20px;" >
			...
			<tr>
				...
				<td><input type="text" class="form-control" id="projectSapCode" value="${repository.projectSapCode}"></td>
			</tr>
			<tr>
				<td><label for="projectName">&nbsp;&nbsp;&nbsp;&nbsp;Project Name</label></td>			
				<td><input type="text" class="form-control" id="projectName" value="${repository.projectName}"></td>
			</tr>
			<tr>
				<td><label for="pmSAPCode">&nbsp;&nbsp;&nbsp;&nbsp;Manager SAP Code</label></td>			
				<td><input type="text" class="form-control" id="pmSAPCode" onblur="fetchManager(this.value)" value="${repository.projectManager.sapCode}"></td>
			</tr>
			<tr>
				<td><label for="pmName">&nbsp;&nbsp;&nbsp;&nbsp;Manager Name</label></td>			
				<td><input type="text" class="form-control" id="projectManager" readonly="readonly" value="${repository.projectManager.fullName}"></td>
			</tr>
			
			<tr>
				<td><label for="repositoryName">&nbsp;&nbsp;&nbsp;&nbsp;Repository Name</label></td>			
				<td><input type="text" class="form-control" id="repositoryName" value="${repository.repositoryName}" readonly="readonly"></td>
			</tr>
			<tr>
				<td><label for="repoUserCount">&nbsp;&nbsp;&nbsp;&nbsp;No of CM tool users</label></td>			
				<td><input type="text" class="form-control" id="cmtoolUserCount" value="${repository.cmtoolUserCount}" ></td>
			</tr>
			<tr>
				...
				<td>
					...
					<div class='input-group date' id='datetimepicker1'>
                    	<input type="text" class="form-control" id="serviceStartDate" value="${repository.serviceStartDate}">
			...
			<tr>
				...
				<td>
					<div class='input-group date' id='datetimepicker2'>
                    	<input type="text" class="form-control" id="serviceEndDate" value="${repository.serviceEndDate}">
                    	<span class="input-group-addon">
                       		<span class="glyphicon glyphicon-calendar"></span>
                    	</span>
                	</div>
		...
		<td width="50%" valign="top">
			...
			<tr>
				...
				<td><input type="text" class="form-control" id="spocSAPCode" onblur="fetchSpoc(this.value)" value="${repository.spoc.sapCode}"></td>
			</tr>
			<tr>
				<td width="47%"><label for="spocName">&nbsp;&nbsp;&nbsp;&nbsp;2.&nbsp;Name</label></td>			
				<td><input type="text" class="form-control" id="spocName" readonly="readonly" value="${repository.spoc.fullName}"></td>
			</tr>
			
			<tr>
				<td><label for="spocEmail">&nbsp;&nbsp;&nbsp;&nbsp;3.&nbsp;Mail Id</label></td>			
				<td><input type="text" class="form-control" id="spocEmail" readonly="readonly" value="${repository.spoc.emailId}"></td>
			</tr>
			...
			<tr>
				<td><label for="spocSAPCode">&nbsp;&nbsp;&nbsp;&nbsp;1.&nbsp;SAP Code</label></td>			
				<td><input type="text" class="form-control" id="alternateApproverSapCode" onblur="fetchAlternateApprover(this.value)" value="${repository.alternateApprover.sapCode}"></td>
			</tr>
			<tr>
				<td width="47%"><label for="spocName">&nbsp;&nbsp;&nbsp;&nbsp;2.&nbsp;Name</label></td>			
				<td><input type="text" class="form-control" id="alternateApproverName" readonly="readonly" value="${repository.alternateApprover.fullName}"></td>
			</tr>
			
			<tr>
				<td><label for="spocEmail">&nbsp;&nbsp;&nbsp;&nbsp;3.&nbsp;Mail Id</label></td>			
				<td><input type="text" class="form-control" id="alternateApproverEmail" readonly="readonly" value="${repository.alternateApprover.emailId}"></td>
			</tr>
			...
			<tr>
				...
				<td valign="top" align="center" style="width: 40%;"> 
					<div class="alert alert-info" role="alert" style="margin-bottom:0px; border-bottom: 1px; border-bottom-color: #bce8f1" align="center">
						User List  
						<a type="button" style="float: right;" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#bulkIdCopyModal">Bulk IDs</a>
					</div> 
					<table>
				...
				<td align="center"  valign="top" style="width: 30%;"> 
					<div class="alert alert-info" role="alert" style="margin-bottom:0px; border-bottom: 1px; border-bottom-color: #bce8f1" align="center">
						...
						<a type="button" style="float: left;" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#bulkGroupCopyModal">Bulk Groups</a>
						
						Access Group Name
						
						<a type="button" style="float: right;" class="btn btn-primary btn-xs" data-toggle="modal" data-target="#bulkGroupCopyModal" onclick="createDefaultGroups()">Default Groups</a>
...
<div class="modal fade" id="bulkIdCopyModal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
    ...
    <div class="modal-content" align="center">
    	...
    	<div class="btn-group" data-toggle="buttons">
		  ...
		  <label class="btn btn-primary btn-sm">
		    <input type="radio" name="bulkIdCopy" id="bulkIdCopy_login_id"> Login ID
		  </label>

		  <label class="btn btn-primary btn-sm">
		    <input type="radio" name="bulkIdCopy" id="bulkIdCopy_sap_code"> SAP Code
		  </label>

		  <label class="btn btn-primary btn-sm">
		    <input type="radio" name="bulkIdCopy" id="bulkIdCopy_email_id"> EMail ID
		  </label>
		  
		</div>
		<hr>
		<textarea class="form-control" id="bulkIdCopy_field" rows="50" style="width: 80%; height: 300px;"></textarea>
		
		<br>
		
		<button type="button" class="btn btn-primary" onclick="bulkAddToUsers()" >Add Users</button>
...
<div class="modal fade" id="bulkGroupCopyModal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
  <div class="modal-dialog">
    <div class="modal-content" align="center">

		<hr>
		<textarea class="form-control" id="bulkGroupCopy_field" rows="50" style="width: 80%; height: 300px;"></textarea>
		
		<br>
		
		<button type="button" class="btn btn-primary" onclick="bulkAddToGroups()" >Add Groups</button>
